...
import { rows, columns, shiftCoordinates, assignLocation, checkCoordinates } from './auxiliary-functions';
...
class Gameboard {
    ...
    dataShips([x,y],direction,size,shipNumber) {
        ...
        if(shipCoordinates===null){
            ...
            let notAvailable = shipCoordinates.map(
                (x) => this.board[shiftCoordinates(x)[0]][shiftCoordinates(x)[1]].value === 1
                );
    ...
    recordedAttack( [x,y], 
        ...
        if(!checkCoordinates([x,y]) || this.arrayAllAttack.has([x,y.toLowerCase()].toString()) ){
            return null;
        }else{
